// Topics grounded in Priyanka's real work — building AI-augmented growth
// systems at Deriv across 15+ regulated markets. Update as her talks evolve.

export const speakingTopics = [
  {
    title: "The MCP Pipeline Behind a $1M/Month Growth Engine",
    description:
      "How a social-listening system gets routed through MCP into real content and campaign priorities. Not vibes. Signal.",
    outcomes: [
      "The actual architecture of a signal-to-content pipeline",
      "Why most 'AI-powered marketing' is a chatbot with extra steps",
    ],
  },
  {
    title: "AI Localization at Scale: Doubling Output Without Doubling Headcount",
    description:
      "The human-in-the-loop system that shipped content across 15+ markets faster, without the brand voice collapsing into generic output.",
    outcomes: [
      "The QA layer that keeps localized content from sounding translated",
      "How brand glossaries stop 15 markets sounding like 15 companies",
    ],
  },
  {
    title: "SEO Is Dying. Here's What GEO Looks Like in Practice.",
    description:
      "AI Overviews and zero-click search broke the old SEO playbook. Here's the schema and intent-cluster architecture that keeps you visible anyway.",
    outcomes: [
      "A practical framework for auditing your GEO readiness",
      "What actually changes in content structure for AI search",
    ],
  },
];

export const mediaFeatures = ["Bloomberg", "CNBC", "MediaCorp", "SPH", "Discovery"];
